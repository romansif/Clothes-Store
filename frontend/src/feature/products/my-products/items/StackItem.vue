...
<template>
  <TransitionGroup name="list">
    <li @click="getProductId(product.productId)" v-for="product in stack" :key="product.id" class="flex gap-8 pr-25 pl-15">
      <div class="flex flex-col">
        <div class="relative">
          ...
          <img @click="toggleToFavorite(product.id, 'cart', product.productId)" :src="product.favorite ? liked : like"
               alt="" :class="['absolute top-0.5 left-74.5 w-[32px]', outOfStack.length > 0 ? '' : 'left-75.5']">
        </div>
      </div>
