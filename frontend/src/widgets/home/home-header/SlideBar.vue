...
<template>
  <div class="relative overflow-hidden w-368">
    <Transition name="slide" mode="out-in">
      <div :key="slides[activeSlide].id" class="bg-black flex justify-between h-90 rounded-2xl">
        <div class="flex flex-col justify-center gap-2 px-25">
          ...
          <div class="flex items-center justify-between mt-8">
            <router-link :to="{name: `${slides[activeSlide].routeName}`}">
        ...
        <img :src="slides[activeSlide].url" alt="" class="opacity-75 rounded-r-2xl" />
      </div>
    </Transition>
  </div>
</template>
