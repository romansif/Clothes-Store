<template>
  <div class="font-montserrat relative overflow-hidden w-368 mt-12">
    <Transition name="slide" mode="out-in">
      <div :key="homeSlides[activeSlide].id" class="bg-black flex justify-between h-96 rounded-2xl">
        <div style="filter: drop-shadow(0 0 8px rgba(255, 255, 255, 1))" class="flex flex-col justify-center gap-2 px-20">
          <div class="flex flex-col gap-1 text-gray-50 w-100">
          ...
          <p class="text-lg mt-2 text-gray-300 break-after-allw-110">
            {{ homeSlides[activeSlide].text }}
          </p>
          <div class="flex items-center justify-between mt-8 w-110">
            <router-link :to="{name: `${homeSlides[activeSlide].routeName}`}" class="transition duration-500 hover:scale-108">
              <span class="text-2xl font-medium text-gray-200 break-after-all border-b-2 cursor-pointer w-fit pb-1.5">
