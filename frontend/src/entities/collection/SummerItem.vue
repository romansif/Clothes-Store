<template>
  <TransitionGroup name="list" mode="out-in">
    <li @click="getProductId(product.id)" v-for="product in summerSelection" :key="product.id" class="flex flex-col w-fit">
      <div class="relative">
        <router-link :to="{ name: 'product/info', params: { id: product.id } }">
          <img :src="productPreview(product.id, summerSelection)" alt=""
        ...
        <img @click="toggleToFavorite(product.id, 'product', product.id)" :src="isFavorite(product.id, userData.id) ? liked : like" alt=""
             class="absolute top-0.5 left-31 w-6.25 cursor-pointer sm:w-8.75 sm:left-58.5 md:left-66.5 lg:left-58.5 xl:left-78">
      </div>
...
const { summerSelection } = productStore();
