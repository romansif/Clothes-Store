<template>
  <TransitionGroup name="list" mode="out-in">
    <li @click="getProductId(product.id)" v-for="product in autumnSelection" :key="product.id" class="flex flex-col w-fit">
      <div class="relative">
        <router-link :to="{ name: 'product/info', params: { id: product.id } }">
          <img :src="productPreview(product.id, autumnSelection)" alt=""
...
const { autumnSelection } = productStore();
