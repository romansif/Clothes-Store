...
export const paymentClasses = () => {
    ...
    const paymentMethodClass = (method: string) => [
        ...
        paymentFormErrors.value.paymentMethod ? 'border-red-500' : ''
    ];

    const cardNumberClass = computed(() => [
        `border border-gray-300 rounded-xl outline-none px-4 py-4 text-xs transition duration-400
        font-dm-sans bg-gray-50 hover:bg-white appearance-none placeholder:text-sm`,
        paymentFormErrors.value.cardNumber ? 'placeholder:text-red-500 border-red-500 placeholder:text-xs' : 'placeholder:text-gray-600'
    ]);
    ...
    const expiryDateClass = computed(() => [
        ...
        paymentFormErrors.value.expiryDate ? 'placeholder:text-red-500 border-red-500 placeholder:text-xs' : 'placeholder:text-gray-600'
    ]);
    ...
    const cardCvvClass = computed(() => [
        ...
        paymentFormErrors.value.cardCvv ? 'placeholder:text-red-500 border-red-500 placeholder:text-xs' : 'placeholder:text-gray-600'
    ]);
