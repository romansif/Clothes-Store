<template>
  <div class="flex flex-col gap-6 bg-white w-full p-6.5 rounded shadow-xl">
    ...
    <div class="flex gap-6">
      <div class="bg-gray-50 h-90 w-90 border border-gray-300 transition duration-400 hover:scale-105
           hover:bg-gray-100 cursor-pointer overflow-hidden rounded-lg"
           @click="openSelectImage(1)" >
        <img v-if="itemPreview[1]"
             :src="itemPreview[1]" alt=""
      ...
      <div class="grid grid-cols-2 gap-3">
        <div v-for="index in [2, 3, 4, 5]" :key="index"
             class="bg-gray-50 h-43.5 w-43.5 border border-gray-300 transition duration-400 hover:scale-105
             hover:bg-gray-100 cursor-pointer overflow-hidden rounded-lg"
             @click="openSelectImage(index)">
          <img v-if="itemPreview[index]"
               :src="itemPreview[index]" alt=""
