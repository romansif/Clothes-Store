<template>
  <Transition name="view">
    <div v-if="springCatalog.length === 0" class="pt-45">
    ...
    <ul v-horizontal-scroll v-else class="flex gap-16.5 mt-5 overflow-x-auto whitespace-nowrap no-scrollbar pt-5">
      <SpringItem v-for="product in springCatalog" :product="product" :spring-catalog="springCatalog" />
    </ul>
  </Transition>
</template>
...
import SpringItem from "@/features/use-season-product/ui/spring/SpringItem.vue";
import empty_products from "@/assets/icons/products/icon-products.svg";

const { vHorizontalScroll } = productsHelper();
const { getFavoriteProducts } = useGetFavorite();
const { getSeasonal, springCatalog } = useGetSeasonProducts();
