<template>
  <TransitionGroup name="list">
    <li v-for="product in summerCatalog" :key="product.id"
        class="flex flex-col shrink-0 lg:w-80 w-50">
      <div class="relative">
        <router-link :to="{ name: 'product/info', params: { id: product.id } }">
          <img :src="productPreview(product.id, summerCatalog)" alt=""
...
const { isOutOfStack, productPreview, summerCatalog } = useGetProduct();
</script>
