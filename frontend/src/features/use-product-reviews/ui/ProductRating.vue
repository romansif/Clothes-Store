  ...
  <div class="flex font-dm-sans gap-15">
    <div class="flex flex-col">
      <div class="flex items-baseline gap-2.5">
        <span class="text-[115px] font-semibold leading-none">{{ averageRating() }}</span>
        <span class="text-3xl leading-none">/</span>
        <span class="text-3xl leading-none">5</span>
      </div>
      <div class="flex items-center">
        <img v-for="index in Math.ceil(averageRating())" :key="index"
             :src="starCountSrc(index, averageRating())" alt="" class="w-12">
      </div>
...
import { reviewModal } from "@/features/use-product-reviews/lib/review-photo-modal.ts";
import {
  reviewsHelper
...
import ReviewPhotos from "@/features/use-product-reviews/ui/ReviewPhotos.vue";

const { reverseRatings } = reviewsStore();
